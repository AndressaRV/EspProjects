...
void setup() {
  ...
  delete_old_stuff(); // checa o espaço no cartão de memoria
  camera_fb_t *fb = NULL;
  ...
  xTaskCreatePinnedToCore( the_camera_loop, "the_camera_loop", 4000, NULL, 6, &the_camera_loop_task, 0); // prio 3, core 0 //v56 core 1 as http dominating 0 ... back to 0, raise prio

  delay(100);

  // prio 4 - higher than the cam_loop(), and the streaming
  xTaskCreatePinnedToCore( the_sd_loop, "the_sd_loop", 3000, NULL, 4, &the_sd_loop_task, 1);  // prio 4, core 1

  delay(200);
}
///// fim do setup
//loop do cartao de memoria
...
void the_camera_loop (void* pvParameter) {
  ...
  while (1) {
    ...
    if (frame_cnt == 0 && start_record == 0) {
      ...
      long wait_for_cam_start = millis();
      fb_next = esp_camera_fb_get();               // should take near zero, unless the sd is faster than the camera, when we will have to wait for the camera
      //if (framebuffer_time < (millis() - 10)){
      xSemaphoreTake( baton, portMAX_DELAY );
      framebuffer_len = fb_next->len;                    // v59.5
      memcpy(framebuffer, fb_next->buf, fb_next->len);   // v59.5
...
static void start_avi() {
  ...
  sprintf(nomePasta, "/Pasta%s%d.%03d", devname, file_group, file_number);
  sprintf(avi_file_name, "%s/%s%d.%03d.avi", nomePasta, devname, file_group, file_number);
  if(SD_MMC.mkdir(nomePasta))
  {
    Serial.println("Consegui abrir a pasta");
  }
...
void createDir(fs::FS &fs, const char * path){ //cria um diretório pro video e pras imagens
